...
test("customer submits a work request and sees it in the list", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/customer\/requests\/new$/, { timeout: 10_000 });
  ...
  await expect(page.getByText("Test request from E2E")).toBeVisible({ timeout: 10_000 });
  await expect(page.getByText("Submitted — awaiting review")).toBeVisible({ timeout: 5_000 });

  await page.click('a:has-text("Requests")');
  await expect(page).toHaveURL(/\/customer\/requests$/, { timeout: 10_000 });
