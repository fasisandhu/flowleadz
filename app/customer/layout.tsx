...
const CUSTOMER_NAV: MobileNavLink[] = [
  ...
  { href: "/customer/projects", label: "Projects", icon: "projects" },
  { href: "/customer/requests", label: "Requests", icon: "requests" },
];
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/85 backdrop-blur dark:border-slate-800 dark:bg-slate-950/85">
        <div className="mx-auto flex h-12 max-w-6xl items-center justify-between gap-4 px-4">
          <div className="flex min-w-0 items-center gap-5">
            ...
            <Link href="/customer/dashboard" className="flex items-center gap-1.5 text-sm font-semibold tracking-tight text-slate-900 dark:text-slate-50">
            ...
            <nav aria-label="Main" className="hidden items-center gap-0.5 text-sm md:flex">
              <Link href="/customer/dashboard" className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/60 dark:hover:text-slate-50">
                <LayoutDashboard className="h-3.5 w-3.5" />
                Dashboard
              </Link>
              <Link href="/customer/projects" className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/60 dark:hover:text-slate-50">
                <FolderKanban className="h-3.5 w-3.5" />
                Projects
              </Link>
              <Link href="/customer/requests" className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/60 dark:hover:text-slate-50">
